...
file_path <- paste0("02_Data/05-variability-both-noise-", amplitude, "-", frequency, ".rds")
...
color_palette <- c(Intercept = "#d14b00", Connectivity = "#0086a7", Abiotic = "#0d2967")
...
df_regression <- dplyr::filter(df_cv_prod, measure %in% c("alpha", "gamma")) %>% 
  ...
  purrr::map_dfr(function(i) {
    ...
    lm_temp <- lm(log(value.cv) ~ move_meta_sd + noise_sd, data = i)
    
    pred <- coef(lm_temp)[[1]] + coef(lm_temp)[[2]] * i$move_meta_sd + coef(lm_temp)[[3]] * i$noise_sd
    ...
    tibble::tibble(
      ...
      int = lm_temp$coefficients[[1]], coef_a = lm_temp$coefficients[[2]],
      coef_b = lm_temp$coefficients[[3]], r.squared = r.squared, p.value = p.value)
  })
...
gg_list <- map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  labels_y_right <- round(seq(from = min(c(df_temp$coef_a, df_temp$coef_b)), 
                              to = max(c(df_temp$coef_a, df_temp$coef_b)), 
                              length.out = 4), digits = 2)
  
  scales_y <- seq(from = min(df_temp$int), to = max(df_temp$int), length.out = 4)
  
  df_temp$coef_a <- scales::rescale(df_temp$coef_a, to = range(df_temp$int))
  
  df_temp$coef_b <- scales::rescale(df_temp$coef_b, to = range(df_temp$int))
  ...
  ggplot(data = df_temp) + 
    ...
    geom_point(aes(x = pop_n, y = coef_a, shape = measure, color = "Connectivity"), size = 3.0) +
    geom_line(aes(x = pop_n, y = coef_a, group = measure, linetype = measure, color = "Connectivity"), alpha = 0.5) +
    geom_point(aes(x = pop_n, y = coef_b, shape = measure, color = "Abiotic"), size = 3.0) +
    geom_line(aes(x = pop_n, y = coef_b, group = measure, linetype = measure, color = "Abiotic"), alpha = 0.5) +
    ...
    theme(legend.position = "none",
          axis.title.y.left = element_text(color = color_palette[["Intercept"]]), axis.title.y.right = element_text(color = color_palette["Connectivity"]), 
          axis.text.y.left = element_text(color = color_palette[["Intercept"]]), axis.text.y.right = element_text(color = color_palette["Connectivity"]),
...
gg_dummy <- ggplot(data = df_regression, aes(x = pop_n)) + 
  ...
  geom_point(aes(y = coef_a, color = "Connectivity", shape = factor(measure, labels = c("Local", "Meta-ecosystem")))) + 
  geom_point(aes(y = coef_b, color = "Abiotic", shape = factor(measure, labels = c("Local", "Meta-ecosystem")))) + 
  scale_shape_manual(name = "Scale", values = c(19, 2)) +
...
suppoRt::save_ggplot(plot = gg_lm_coef, filename = paste0("06-lm-coeff-", amplitude, extension),
