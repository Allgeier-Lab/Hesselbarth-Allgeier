...
foo_hpc <- function(pop_n, move_meta_sd) {
  
  library(dplyr)
  
  list_starting$pop_n <- pop_n
  
  # update move meta * parameters
  ...
  list(moved = dplyr::mutate(moved, pop_n = pop_n, move_meta_sd = move_meta_sd), 
       cv = dplyr::mutate(dplyr::bind_rows(cv), pop_n = pop_n, move_meta_sd = move_meta_sd), 
       production = dplyr::mutate(production, pop_n = pop_n, move_meta_sd = move_meta_sd))
...
sbatch_cv <- rslurm::slurm_apply(f = foo_hpc, params = experiment_df, 
                                 global_objects = globals, jobname = "move_cv",
...
suppoRt::save_rds(object = cv_result, 
                  filename = paste0("05-variability-move-", 
