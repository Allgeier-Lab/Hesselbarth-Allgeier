...
# Purpose: Results of connectivity experiment for movement variability
...
file_path <- paste0("02_Data/05-variability-move-", amplitude, "-", frequency, ".rds")

df_cv_prod <- readr::read_rds(file_path) %>% 
  purrr::map_dfr(function(j) {
    dplyr::left_join(x = j$cv, y = j$production, by = c("part", "pop_n", "move_meta_sd"), 
                     suffix = c(".cv", ".prod")) %>% 
...
#### Parameter vs. movement ####

gg_param_move <- dplyr::filter(df_cv_prod, measure == "alpha") %>% 
  ggplot(aes(x = move_meta_sd, y = value.move, color = pop_n)) + 
  scale_color_manual(name = "Population size", values = colors_pop) + 
  geom_point(shape = 19, alpha = 0.5) +
  geom_smooth(formula = "y ~ x", se = FALSE, method = "loess") + 
  scale_x_continuous(breaks = seq(from = 0.1, to = 1.0, by = 0.1)) +
  labs(x = expression(italic(move_meta_sd)), y = "Mean cross-ecosystem movement") +
  theme_classic(base_size = base_size) +
...
list_gg_parts <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  df_regression <- dplyr::filter(df_cv_prod, part == part_i, measure %in% c("alpha", "gamma")) %>% 
    ...
    purrr::map_dfr(function(i) {
      ...
      lm_temp <- lm(log(value.cv) ~ move_meta_sd, data = i)
      
      pred <- coef(lm_temp)[[1]] + coef(lm_temp)[[2]] * i$move_meta_sd
      ...
      tibble::tibble(measure = unique(i$measure), pop_n = unique(i$pop_n),
                     move_meta_sd = i$move_meta_sd, value.cv = log(i$value.cv), value.cv.pred = pred, 
                     int = lm_temp$coefficients[[1]], coef = lm_temp$coefficients[[2]],
                     r.squared = r.squared, p.value = p.value, 
                     x_label = mean(move_meta_sd), # min(move_meta_sd) + (max(move_meta_sd) - min(move_meta_sd)) * 0.05,
                     y_label = mean(value.cv.pred)) # min(value.cv.pred) + (max(value.cv.pred) - min(value.cv.pred)) * 0.05)
    })
  
  df_label <- dplyr::select(df_regression, -c(move_meta_sd, value.cv, value.cv.pred)) %>%
    dplyr::group_by(measure, pop_n) %>%
    dplyr::distinct() %>%
  ...
  ggplot(data = df_regression) +
    geom_point(aes(x = move_meta_sd, y = value.cv, color = pop_n), shape = 19, alpha = 0.25) +
    geom_line(aes(x = move_meta_sd, y = value.cv.pred, color = pop_n)) +
    geom_label(data = df_label, aes(x = x_label, y = y_label, label = label, group = pop_n),
               parse = TRUE, size = text_size) +
    facet_wrap(. ~ measure, nrow = 1, labeller = labeller(measure = labeller_measure), scales = "free_y") + 
    scale_color_manual(name = "Population size", values = colors_pop) +
    scale_x_continuous(breaks = seq(from = 0.1, to = 1.0, by = 0.1)) +
    scale_y_continuous(breaks = function(x) seq(quantile(x, 0.1), quantile(x, 0.9), length.out = 4),
                       limits = function(x) c(min(x), max(x)), labels = function(x) round(exp(x), 3)) +
    labs(x = ifelse(test = part_i == "ttl_production", yes = expression(italic(move_meta_sd)), no = ""), 
...
suppoRt::save_ggplot(plot = gg_move_cv_overall, filename = paste0("06-move-cv-", amplitude, extension),
                     path = "04_Figures/", width = width, height = height / 2,
                     units = units, dpi = dpi, overwrite = overwrite)

#### CV vs. PP (log-log) ####

df_regression <- dplyr::group_by(df_cv_prod, pop_n, part, measure) %>%
  dplyr::group_split() %>%
  purrr::map_dfr(function(i) {
    ...
    tibble::tibble(part = unique(i$part), measure = unique(i$measure), pop_n = unique(i$pop_n), 
                   ...
                   int = lm_temp$coefficients[[1]], coef = lm_temp$coefficients[[2]],
                   r.squared = r.squared, p.value = p.value, 
                   x_label = mean(value.cv), y_label = mean(value.prod.pred))
    
  })

df_label <- dplyr::select(df_regression, -c(value.cv, value.prod, value.prod.pred)) %>% 
...
list_gg_parts <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  dplyr::filter(df_regression, part == part_i) %>% 
    ...
    geom_line(aes(x = value.cv, y = value.prod.pred, color = pop_n)) +
    # geom_label(data = dplyr::filter(df_label, part == part_i),
    #            aes(x = x_label, y = y_label, label = label, group = pop_n), 
    #            parse = TRUE, size = text_size) +
    ...
    scale_y_continuous(breaks = function(x) seq(quantile(x, 0.1), quantile(x, 0.9), length.out = 5), 
                       limits = function(x) c(min(x), max(x)), labels = function(x) round(exp(x), 1)) +
    guides(color = guide_legend(override.aes = list(shape = 19, alpha = 1.0), 
                                nrow = 2, byrow = TRUE)) +
    labs(x = ifelse(test = part_i == "ttl_production", yes = "Coeffiecent of variation", no = ""),
         y = ifelse(test = part_i == "bg_production", yes = "Total primary production", no = "")) +
    theme_classic(base_size = base_size) + 
...
suppoRt::save_ggplot(plot = gg_cv_prod_overall, filename = paste0("06-move-cv-prod-", amplitude, extension),
