...
# Purpose: Simulation experiment of noise_sd and CV/Production
...
experiment_df <- expand.grid(pop_n = c(8, 16, 32, 64),
                             noise_sd = seq(from = 0.1, to = 1, by = 0.1)) %>%
...
foo_hpc <- function(pop_n, noise_sd) {
  ...
  list(moved = dplyr::mutate(moved, pop_n = pop_n, noise_sd = noise_sd), 
       cv = dplyr::mutate(dplyr::bind_rows(cv), pop_n = pop_n, noise_sd = noise_sd), 
       production = dplyr::mutate(production, pop_n = pop_n, noise_sd = noise_sd))
...
sbatch_cv <- rslurm::slurm_apply(f = foo_hpc, params = experiment_df, 
                                 global_objects = globals, jobname = "noise_cv",
...
suppoRt::save_rds(object = cv_result, 
                  filename = paste0("05-variability-both-noise-", 
