...
# Purpose: Results of connectivity experiment for noise variability
...
file_path <- paste0("02_Data/05-variability-noise-", amplitude, "-", frequency, ".rds")

df_cv_prod <- readr::read_rds(file_path) %>% 
  purrr::map_dfr(function(j) {
    dplyr::left_join(x = j$cv, y = j$production, by = c("part", "pop_n", "noise_sd"), 
                     suffix = c(".cv", ".prod")) %>% 
...
gg_density <- dplyr::filter(df_cv_prod, measure %in% c("alpha", "gamma")) %>% 
  ...
  facet_wrap(. ~ part + measure, scales = "free_y", ncol = 2, nrow = 3, 
...
list_gg_parts <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  df_regression <- dplyr::filter(df_cv_prod, part == part_i, measure %in% c("alpha", "gamma")) %>% 
    ...
    purrr::map_dfr(function(i) {
      ...
      lm_temp <- lm(log(value.cv) ~ noise_sd, data = i)
      
      pred <- coef(lm_temp)[[1]] + coef(lm_temp)[[2]] * i$noise_sd
      ...
      tibble::tibble(measure = unique(i$measure), pop_n = unique(i$pop_n),
                     noise_sd = i$noise_sd, value.cv = log(i$value.cv), value.cv.pred = pred, 
                     int = lm_temp$coefficients[[1]], coef = lm_temp$coefficients[[2]],
                     r.squared = r.squared, p.value = p.value, 
                     x_label = mean(noise_sd), # min(noise_sd) + (max(noise_sd) - min(noise_sd)) * 0.05,
                     y_label = mean(value.cv.pred)) # min(value.cv.pred) + (max(value.cv.pred) - min(value.cv.pred)) * 0.05)
    })
  
  df_label <- dplyr::select(df_regression, -c(noise_sd, value.cv, value.cv.pred)) %>%
    dplyr::group_by(measure, pop_n) %>%
    dplyr::distinct() %>%
  ...
  ggplot(data = df_regression) +
    geom_point(aes(x = noise_sd, y = value.cv, color = pop_n), shape = 19, alpha = 0.25) +
    geom_line(aes(x = noise_sd, y = value.cv.pred, color = pop_n)) +
    ...
    scale_y_continuous(breaks = function(x) seq(quantile(x, 0.1), quantile(x, 0.9), length.out = 5),
                       limits = function(x) c(min(x), max(x)), labels = function(x) round(exp(x), 2)) +
    labs(x = ifelse(test = part_i == "ttl_production", yes = expression(italic(noise_sd)), no = ""), 
...
list_gg_parts <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  dplyr::filter(df_regression, part == part_i) %>% 
    ...
    #            aes(x = x_label, y = y_label, label = label), parse = TRUE, size = text_size) +
    facet_wrap(. ~ measure + part, nrow = 3, ncol = 3, scales = "free",
...
suppoRt::save_ggplot(plot = gg_cv_prod_overall, filename = paste0("06-noise-cv-prod-", amplitude, extension),
