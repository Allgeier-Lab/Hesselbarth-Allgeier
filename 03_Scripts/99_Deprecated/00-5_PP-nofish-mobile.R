...
sbatch_pp <- rslurm::slurm_apply(f = foo, params = df_experiment,
                                 global_objects = globals, jobname = "PP_nofish_mobile",
...
# get results as list
pp_nofish_mobile <- rslurm::get_slurm_out(sbatch_pp, outtype = "table")

# save results to disk
suppoRt::save_rds(object = pp_nofish_mobile, file = "02_Data/00_pp_nofish_mobile.rds", 
                  overwrite = overwrite)
...
pp_nofish_mobile <- readr::read_rds("02_Data/00_pp_nofish_mobile.rds") %>%
  dplyr::mutate(reldiff = ((mobile - nofish) / nofish) * 100) %>%
...
y_labels <- list("", expression(paste(Delta, "no fish, mobile fish [%]")), "")

gg_list <- map(seq_along(parts_list), function(i) {
  
  dplyr::filter(pp_nofish_mobile, fun == "sum", name %in% parts_list[[i]]) %>% 
    ggplot() +
...
# dplyr::group_by(pp_nofish_mobile, fun, name, enrichment, amplitude) %>%
...
y_labels <- list("", "log(RR) no fish, mobile fish", "")
