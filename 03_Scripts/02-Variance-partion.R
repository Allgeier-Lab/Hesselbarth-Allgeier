...
cv_both <- readr::read_rds("02_Data/01-both-variability.rds") %>% 
...
variance_partion <- dplyr::filter(cv_both, part %in% c("ag_production", "bg_production", "ttl_production"),
                                  measure %in% c("alpha", "gamma", "beta"), stochastic == "both") %>% 
  dplyr::group_by(part, measure, amplitude_mean) %>% 
  dplyr::group_split() %>% 
  purrr::map_dfr(function(i) {
    
    part_temp <- vegan::varpart(i$value, ~ amplitude_sd, ~ phase_sd, ~ residence_sd, data = i)
    
    tibble::tibble(part = unique(i$part), measure = unique(i$measure), amplitude_mean = unique(i$amplitude_mean),
                   explanatory = c("amplidue", "phase", "residence", "amplitude_phase", 
                                   "phase_residence", "amplitude_residence", "amplitude_phase_residence", 
                                   "residuals"), 
...
df_circle <- tibble::tibble(x = rep(x = c(0, 0.866, -0.866), times = 3),
                            ...
                            explanatory = rep(x = c("Ampliude", "Phase", "Residence"), times = 3))
...
gg_var_partion <- ggplot() + 
  ...
  annotate(geom = "text", x = -2.75, y = 0, angle = 90, color = "#f2af35", label = "Residence", ) +
  scale_fill_manual(name = "", values = c("#a71c00", "#138cc1", "#f2af35")) +
