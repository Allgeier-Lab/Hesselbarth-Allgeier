...
full_lm_df <- purrr::map_dfr(results_combined_list, function(df_temp) {
  ...
  lm_temp <- lm(value.prod ~ abiotic * biotic, data = df_temp) 
...
suppoRt::save_ggplot(plot = gg_gamma$noise$ag, 
                     filename = ifelse(near, yes = "Figure-4-near.pdf", no = "Figure-4.pdf"),
                     path = "04_Figures/", width = width, height = height * 0.5,
                     units = units, dpi = dpi, overwrite = overwrite)

suppoRt::save_ggplot(plot = gg_gamma$noise$ag, 
                     filename = ifelse(near, yes = "Figure-A8-near.pdf", no = "Figure-A8.pdf"),
                     path = "04_Figures/Appendix/", width = width, height = height * 0.5,
                     units = units, dpi = dpi, overwrite = overwrite)

# suppoRt::save_ggplot(plot = gg_gamma$noise$bg, filename = "Figure-A4.pdf",
#                      path = "04_Figures/Appendix/", width = width, height = height * 0.5,
#                      units = units, dpi = dpi, overwrite = overwrite)
