...
# results_combined_list <- dplyr::filter(results_combined_df, measure == "beta",
...
gg_coef_scenario <- purrr::map(c(phase = "phase", noise = "noise"), function(scenario_i) {
  ...
  range_estimate <- dplyr::filter(full_lm_df, scenario == scenario_i, term == "value.cv") |> 
    dplyr::pull(estimate) |> 
    range() |> 
    abs() |> 
    max()
  ...
  purrr::map(c(ag_production = "ag_production", bg_production = "bg_production", ttl_production = "ttl_production"), function(part_j) {
    ...
    dplyr::filter(full_lm_df, scenario == scenario_i, part == part_j, term == "value.cv") |> 
      
      ggplot() +
      
      # adding geoms
      geom_hline(yintercept = 0.0, linetype = 2, color = "grey") +
      geom_linerange(aes(x = pop_n, ymin = 0.0, ymax = estimate),
                     alpha = 0.25, position = position_dodge(width = width_pos), linewidth = size_line) +
      geom_point(aes(x = pop_n, y = estimate, shape = direction),
      ...
      # facet gridding
      # facet_grid(rows = dplyr::vars(nutrient_input), scales = "free_y",
      #            labeller = labeller(nutrient_input = function(x) paste0("Nutr. input: ", x))) +
      facet_wrap(. ~ nutrient_input, nrow = 3, scales = "free_x",
                 labeller = labeller(nutrient_input = function(x) paste0("Nutr. input: ", x))) +
