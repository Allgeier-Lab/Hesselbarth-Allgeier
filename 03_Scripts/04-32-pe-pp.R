...
gg_part <- purrr::map(c(Aboveground = "Aboveground", Total = "Total"), function(part_i) {
  ...
  df_temp <- dplyr::filter(results_pe_pp_df, part == part_i, measure == "alpha") |> 
  ...
  rel_imp_temp <- dplyr::filter(rel_importance_df, part == part_i, 
                                measure == "Local scale", beta != "Residuals")
  
  lm_temp <- dplyr::filter(lm_summary_df, part == part_i, measure == "alpha") |> 
    dplyr::slice(1) |> 
...
gg_pe_pp <- cowplot::plot_grid(gg_part$Aboveground, gg_part$Total, nrow = 2) |> 
  ...
  cowplot::draw_label(expression(paste("Coeffiecent of variation ", italic(alpha))), 
...
suppoRt::save_ggplot(plot = gg_pe_pp, filename = "Figure-4-alpha.pdf",
                     path = "04_Figures/", width = width, height = height * 1/2,
                     units = units, dpi = dpi, overwrite = overwrite)

suppoRt::save_ggplot(plot = gg_pe_pp, filename = "Figure-4-alpha.jpg",
                     path = "04_Figures/", width = width, height = height * 1/2,
                     units = units, dpi = dpi, overwrite = overwrite)
