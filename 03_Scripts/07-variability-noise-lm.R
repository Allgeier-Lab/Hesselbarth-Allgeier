...
gg_list <- map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  ggplot(data = df_temp) + 
    ...
    geom_point(aes(x = pop_n, y = coef_abiotic, shape = measure, color = "Noise"), size = 3.0) +
    geom_line(aes(x = pop_n, y = coef_abiotic, group = measure, linetype = measure, color = "Noise"), alpha = 0.5) +
...
gg_dummy <- ggplot(data = df_regression, aes(x = pop_n)) + 
  ...
  geom_point(aes(y = coef_abiotic, color = "Noise", shape = factor(measure, labels = c("Local", "Meta-ecosystem")))) + 
  scale_shape_manual(name = "Scale", values = c(19, 2)) +
