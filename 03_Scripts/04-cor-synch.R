...
gg_var_synch <- purrr::map(c("phase", "noise"), function(i) {
  ...
  gg_part <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(j) {
    ...
    gg_pop_n <- purrr::map(c(8, 16, 32, 64, 128), function(k) {
      
      data_temp <- dplyr::filter(results_var_synch_df, scenario == i, part == j, pop_n == k)
      
      label_part <- ""
      
      label_pop_n <- ""
      
      if (j == "ag_production" & k == 8) label_part <-  "Aboveground"
      if (j == "bg_production" & k == 8) label_part <-  "Belowground"
      if (j == "ttl_production" & k == 8) label_part <- "Total         "
      
      if (j == "ag_production") label_pop_n <- paste0("Population size: ", k)
      
      ggplot(data = data_temp, aes(x = value.var, y = value.cv, color = variability, group = variability)) + 
        ...
        theme_classic(base_size = size_base) + 
        theme(legend.position = "none", plot.title = element_text(size = size_base * 0.8), 
              plot.subtitle = element_text(size = size_base * 0.8), 
              plot.margin = unit(c(t = 1.0, r = -2.5, b = -2.5, l = -2.5), "mm"))
      
    })
    
    cowplot::plot_grid(plotlist = gg_pop_n, ncol = 5)
...
gg_synch_cv <- purrr::map(c("phase", "noise"), function(i) {
  ...
  gg_part <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(j) {
    ...
    gg_pop_n <- purrr::map(c(8, 16, 32, 64, 128), function(k) {
      ...
      data_temp <- dplyr::filter(results_cv_synch_df, scenario == i, part == j, pop_n == k)
      ...
      ggplot(data = data_temp, aes(x = synchrony, y = value, color = measure, group = measure)) + 
        ...
        scale_y_continuous(limits = function(x) range(x), 
                           breaks = function(x) seq(quantile(x, 0.1), quantile(x, 0.9), length.out = 4), 
                           labels = function(x) round(x, 3)) +
        scale_color_manual(name = "", values = c("alpha" = "#cbcbcb", "gamma" = "#32b2da"), 
                           labels = c("Local scale", "Meta-ecosystem scale")) +
