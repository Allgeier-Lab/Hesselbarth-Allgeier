...
gg_pop_prod_agbg <- dplyr::filter(df_cv_prod, part %in% c("ag_production", "bg_production"),
  ...
  geom_jitter(alpha = 0.1, shape = 19) +
  ...
  theme(legend.position = "none", panel.border = element_rect(size = 0.5,fill = NA),
...
list_gg_parts <- purrr::map(c("ag_production", "bg_production", "ttl_production"), function(part_i) {
  ...
  list_gg_measures <- purrr::map(c("alpha", "gamma"), function(measure_i) {
    ...
    gg_move_move <- dplyr::filter(df_cv_prod, part == part_i, measure == measure_i) %>% 
      ...
      scale_x_continuous(breaks = seq(0.2, 0.8, 0.2)) + scale_y_continuous(breaks = seq(0.2, 0.8, 0.2)) +
      labs(x = expression(italic(move_meta_mean)), 
           y = ifelse(test = measure_i == "alpha", yes = expression(italic(move_meta_sd)), no = "")) +
      coord_equal() +
      theme_classic(base_size = 8) +
      theme(legend.position = "none", plot.margin = unit(c(2.5, 2.5, 0.0, 2.5), "mm"), # t, r, b, l
    ...
    df_label <- dplyr::select(df_regression, -value.move, -value.cv.log.pred) %>%
      ...
      dplyr::bind_cols(dplyr::filter(df_cv_prod, part == part_i, measure == measure_i) %>% 
                         dplyr::summarise(x = min(value.move) + (max(value.move) - min(value.move)) * c(0.75, 0.9, 0.75, 0.9), 
                                          y = min(value.cv.log) + (max(value.cv.log) - min(value.cv.log)) * c(0.9, 0.9, 0.8, 0.8)))
    ...
    gg_move_cv <- dplyr::filter(df_cv_prod, part == part_i, measure == measure_i) %>% 
      ...
      geom_text(data = df_label, aes(x = x, y = y, label = label), size = 1.75, parse = TRUE) +
      ...
      theme_classic(base_size = 8) + 
      theme(legend.position = "none", plot.margin = unit(c(0.0, 2.5, 2.5, 2.5), "mm"), # t, r, b, l
  ...
  labels <- ifelse(test = c(part_i == "ag_production", part_i == "ag_production"), 
                   yes = c("Local scale", "Meta scale"), no = c("", ""))
  
  cowplot::plot_grid(plotlist = list_gg_measures,  nrow = 1, ncol = 2, 
                     labels = labels, hjust = c(-2.5, -2.5), 
...
suppoRt::save_ggplot(plot = gg_move_cv_overall, filename = paste0("01-mobility-cv", extension),
...
df_label <- dplyr::select(df_regression, -value.cv.log, -value.prod.log.pred) %>% 
  ...
  dplyr::left_join(x = ., y = dplyr::group_by(df_cv_prod, part, measure) %>% 
                     dplyr::summarise(pop_n = factor(c(8, 16, 32, 64), ordered = TRUE), 
                                      x = min(value.cv.log) + 
                                        (max(value.cv.log) - min(value.cv.log)) * c(0.75, 0.9, 0.75, 0.9), 
...
gg_cv_prod_overall <- ggplot(data = df_cv_prod, aes(x = value.cv.log, y = value.prod.log, color = pop_n)) +
  ...
  geom_text(data = df_label, aes(x = x, y = y, color = pop_n, label = label), parse = TRUE, size = 1.75) +
  ...
  theme_classic(base_size = 10) + 
  theme(legend.position = "bottom", strip.text = element_text(hjust = 0.0), 
