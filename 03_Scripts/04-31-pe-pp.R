...
gg_part <- purrr::map(c(Aboveground = "Aboveground", Total = "Total"), function(part_i) {
  ...
  df_temp <- dplyr::filter(results_pe_pp_df, part == part_i, measure == "beta") |> 
  ...
  rel_imp_temp <- dplyr::filter(rel_importance_df, part == part_i, 
                                measure == "Portfolio effect", beta != "Residuals")
  
  lm_temp <- dplyr::filter(lm_summary_df, part == part_i, measure == "beta") |> 
    dplyr::slice(1) |> 
...
write.table(x = lm_summary_df, file = "04_Figures/Table-Q3-full", sep  = ";",
            dec = ".", row.names = FALSE)
...
suppoRt::save_ggplot(plot = gg_pe_pp, filename = "Figure-4.jpg",
                     path = "04_Figures/", width = width, height = height * 1/2,
                     units = units, dpi = dpi, overwrite = overwrite)
